...
| `content/artifacts/<slug>.md` — one of six kinds (`prompt`, `rule`, `skill`, `hook`, `memory`, `settings`), declared in frontmatter | a prompt verbatim as used (sanitised), one line on what it was for, and whether it is still used; for the other five kinds, excerpts ≤40 lines each from a real file, plus a *Where it came from* line naming the tree and stating whether a reader can verify it; a date per entry where the source has one, else one page-level `captured:` range plus an explicit "undated in source" note | improved/idealised rewrites presented as the original; an inferred date; a guessed still-used status; a claimed public address for a private or per-machine file |
...
| `content/atoms/<rung>--<slug>.md` | one claim plus **exactly one** verbatim line in `## Evidence`, that line carrying its own bracketed date; required facets `topic` (must resolve to a `content/topics/` file) · `rung` · `concern` · `question` · `audience[]` · `evidence` · `minutes` · `level` · `source_chapter`; ~15 lines, slide-ready as written; ≤1 image from `content/media/` naming its origin | a second quote block; an Estonian source line quoted in the original (translate and mark it); an atom seeded from a `tier: private` source row; a `minutes` value defended as accurate — it is an estimate and gates nothing |
| `content/course/skeleton.md` | module title · learning outcome · which chapters it draws on | syllabus detail, grading, schedule |
| `content/course/worksheet.md` | the attendee climb-one-rung sheet (circle-your-rung, the two boxes) plus one worked answer; the six-rung ladder table (`from`/`unit` columns from `RUNG_META`) | grading, a schedule, a claimed already-used-in-a-lecture status |
| `content/timeline.json` | nothing by hand — regenerate | any manual edit |
| `content/deck.json` | the lecture's ordered running-order slots (`id` · `title` · `minutes` · `fallback`), mirroring the 90-min order in `journey-corpus.md`; **hand-edited**, unlike `timeline.json` | a `minutes` sum ≠ 90, a `fallback` outside the `deck: true` chapter set, a slot id absent from `SLOTS` in `src/content.config.ts` |
