...
| `content/atoms/<rung>--<slug>.md` | one claim plus **exactly one** verbatim line in `## Evidence`, that line carrying its own bracketed date; required facets `topic` (must resolve to a `content/topics/` file) · `rung` · `concern` · `question` · `audience[]` · `evidence` · `minutes` · `level` · `source_chapter`; ~15 lines, slide-ready as written | a second quote block; an Estonian source line quoted in the original (translate and mark it); an atom seeded from a `tier: private` source row; a `minutes` value defended as accurate — it is an estimate and gates nothing |
