...
1. **Governance root** — the always-loaded instructions that set the repo's non-negotiables.
2. **Path-gated rule** — a convention that loads only when you touch the files it governs.
3. **Shipped plan doc** — how a change is specified before it is built, alternatives-rejected included.
4. **Reviewer-caught defect** — what an independent review sub-agent found that the author had not.
5. **Test / red-demo discipline** — the rule that a new assertion must be shown to fail first.
6. **Postmortem** — what the repo records after something breaks.
